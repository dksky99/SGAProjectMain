...
void APlayerCharacter::UpdateStratagemEtaUI()
{
	...
	// 화면 중앙이 가리키는 월드 위치 (에임 높이와 비교용)
	const FVector aimPoint = GetCenterLoc();

	// "스트라타젬을 보고 있다"로 인정할 최대 가로 거리 (픽셀 기준)
	const float maxScreenDistanceX = 40.0f; // 튜닝용
	...
	// 1차 루프: "화면 중앙에 가장 가까운" 스트라타젬 찾기 (스크린 기준)
	for (TActorIterator<AStratagem> it(world); it; ++it)
	{
		...
		// ETA 계산이 아직 안 됐거나 이미 끝난 스트라타젬은 무시
		if (!stratagem->IsEtaReady() || stratagem->GetImpactRemain() <= 0.0f)
		{
			continue;
		}

		// 스트라타젬보다 아래쪽을 보고 있다면 무시 (빛기둥이 안 보이는 상황)
		...
		// 일정 거리 밖에 있으면 "보고 있는 스트라타젬" 후보에서 탈락
		if (absDx > maxScreenDistanceX)
		{
			continue;
		}

		// 여기까지 왔으면 "에임 근처에 있는 스트라타젬"이므로
		// 이제 그 중에서 가장 중앙에 가까운 놈을 고른다.
	...
	// 후보 중에 "보고 있다고 인정할 만한" 스트라타젬이 하나도 없으면 종료
